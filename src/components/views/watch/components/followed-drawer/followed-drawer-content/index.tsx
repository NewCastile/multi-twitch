...
import useSearchFollowed from "@/hooks/use-search-followed";
...
const FollowedDrawerContent = () => {
  ...
  const getFollowedBasicInfo = (followed: FollowedEntity) => {
    if (isFollowedChannel(followed)) {
      const { broadcaster_login, broadcaster_name } = followed;

      return { broadcaster_login, broadcaster_name };
    } else {
      const { user_login: broadcaster_login, user_name: broadcaster_name } = followed;
